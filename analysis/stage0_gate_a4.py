...
from orthosteric.data.snapshots._builder import CorpusSnapshotV2, SnapshotManifestV2
...
for f in [
    ...
    "scaffold_families_in_largest_component",
]:
    print(f"  {f}: {getattr(ep, f)}")

mo = SnapshotManifestV2(
    schema_version=manifest["schema_version"],
    snapshot_sha256=sha,
    build_provenance_sha256=manifest["build_provenance_sha256"],
    snapshot_id=manifest["snapshot_id"],
    parent_snapshot_sha256=manifest.get("parent_snapshot_sha256"),
    created_at_utc=manifest["created_at_utc"],
    record_count=manifest["record_count"],
    accepted_count=manifest["accepted_count"],
    excluded_count=manifest["excluded_count"],
    censored_count=manifest["censored_count"],
    unresolved_count=0,
    conflict_count=0,
    rule_missing_count=0,
    governance_exception_count=0,
    structural_records_total=0,
    structural_experimental_pdb=0,
    structural_alphafold_fallback=0,
    structural_inadmissible=0,
...
print("  Dimensions:")
for k, v in sorted(res.gate_decision.dimension_summary.items()):
    print(f"    {k}: {v}")

out = {
    "snapshot_sha256": res.snapshot_sha,
    "eligibility": res.eligibility.value,
    "gate_status": res.gate_decision.status.value,
    "gate_rationale": res.gate_decision.rationale,
    "eligible_for_training": res.eligible_for_training,
    "lifecycle_stage": res.lifecycle_stage.value,
    "dimension_summary": res.gate_decision.dimension_summary,
    ...
    "graph_stats": {
        ...
        "per_isoform": gs.per_isoform_compounds,
        "compounds_all4": gs.compounds_all4_isoforms,
        "within_study_four_isoform": gs.within_study_four_isoform,
        "largest_cc": gs.largest_connected_component,
        "n_components": gs.n_connected_components,
