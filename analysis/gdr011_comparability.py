...
for r in acc:
    ...
    r["_bao"] = bf; r["_atype"] = at; r["_desc"] = desc
...
def k_assay(r):    return (r.get("study_id"), r.get("assay_id"))
def k_doc(r):      return (r.get("study_id"),)
def k_proto(r):    return (r.get("study_id"), r["_bao"], r["_atype"])
def k_proto_atp(r):
...
def analyse(name, keyfn):
    ...
    for g, cm in grp_cmpd_iso.items():
        ...
        for ik, isos in cm.items():
            ...
            if isos >= T1:
                complete_cmpds.add(ik); has4 = True
            k = len(isos & T1)
            pair_panels += k*(k-1)//2
        if has4: complete_groups += 1
    ...
    return dict(name=name, groups=len(grp_iso), complete_groups=complete_groups,
        complete_cmpds=len(complete_cmpds), pair_panels=pair_panels,
        hist={k:n_iso_hist[k] for k in (1,2,3,4)}, scaffolds=len(scaf), docs=len(docs),
        cens=dict(cens), iso_bal=dict(iso_bal), comp_recs=len(comp_recs),
...
for r in res:
    print(f"{r['name']:<36}{r['groups']:>8}{r['complete_groups']:>12}{r['complete_cmpds']:>16}{r['pair_panels']:>10}{r['scaffolds']:>11}{r['docs']:>7}")

print("\n=== Isoform-count histogram per (compound x group) ===")
print(f"{'definition':<36}{'1/4':>9}{'2/4':>9}{'3/4':>9}{'4/4':>9}")
for r in res:
    h=r['hist']; print(f"{r['name']:<36}{h[1]:>9}{h[2]:>9}{h[3]:>9}{h[4]:>9}")

print("\n=== Complete-subset composition (bias descriptors) ===")
for r in res:
    if r['complete_cmpds']==0:
        print(f"  {r['name']}: EMPTY"); continue
    print(f"  {r['name']}")
    print(f"      records={r['comp_recs']}  scaffolds={r['scaffolds']}  docs={r['docs']}  ATP-known={r['atp_known_pct']:.1f}%")
...
for i in range(len(names)):
    for j in range(len(names)):
        if i<j:
            ...
            if a or b:
                print(f"  |{names[i][:22]}|={len(a):5}  |{names[j][:22]}|={len(b):5}  shared={len(a&b):5}  A-only={len(a-b):5}  B-only={len(b-a):5}")
...
print(f"  records={len(acc)} compounds={len({r.get('inchikey') for r in acc if r.get('inchikey')})} scaffolds={len(all_scaf)} docs={len({r.get('study_id') for r in acc})}")
