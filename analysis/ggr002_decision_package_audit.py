...
from orthosteric.data.comparability import resolve_panel_key, atp_confirmed_panel_key
...
print(f"GraphStats.compounds_all4_isoforms (global, activity_value-based, "
...
print(f"StratumReport.total_complete_compounds (SUM over {len(c1_strata)} C1 "
...
print(f"\nRECONCILIATION:")
print(f"  activity_value-based unique-compound count (via strata):     {unique_complete_via_strata}")
print(f"  activity_value-based global GraphStats count:                 {gs.compounds_all4_isoforms}")
print(f"  activity_value-based panel-SUMMED count (StratumReport):      {strata.total_complete_compounds}")
...
print(f"  (Expected nonzero: GraphStats counts a compound complete if it has >=1")
print(f"   record in each isoform ANYWHERE across ALL panels combined -- i.e. it")
print(f"   does NOT require the four measurements to come from the SAME panel.")
print(f"   StratumReport requires all four within ONE panel. A compound measured")
print(f"   in alpha in panel P1 and beta/gamma/delta in panel P2 counts toward")
print(f"   GraphStats's 2,500 but toward NEITHER panel's StratumReport count.)")
...
cens_without_pact = Counter(r.get("censoring") for r in acc if pact(r) is None and r.get("activity_value") is not None)
print(f"\nCensoring distribution AMONG records WITH a usable pact() value:    {dict(cens_with_pact)}")
print(f"Censoring distribution AMONG records WITH activity_value but NO pact(): {dict(cens_without_pact)}")
print(f"\n=> ggr_reassessment_a4.py's pact()-based analysis (GGR-002a AND GGR-002b)")
print(f"   silently excludes every record without a pchembl_value -- this is where")
print(f"   right/left-censored ChEMBL records are actually dropped, NOT via an")
print(f"   explicit 'exclude censored' step. This is conservative (never treats")
print(f"   censored as exact) but is an IMPLICIT exclusion, not a documented rule.")
...
for r in acc:
    ...
    if not resolved.is_scientific_evidence: continue
    p = pact(r)
...
print(f"(panel, compound, isoform) cells with >=2 pact-bearing records: {len(multi_cell)} "
      f"of {len(cell_counts)} total cells")
if multi_cell:
    spreads = [max(v) - min(v) for v in multi_cell.values()]
    print(f"  Of these, max-min spread > 0.3 pAct units (i.e. picking a different")
    print(f"  record would give a materially different value): "
    ...
    print(f"  groups' reported by ggr_reassessment_a4.py's GGR-002a index -- but in")
    print(f"  ggr_reassessment_a4.py's GGR-002a pair-generation code, only ONE value")
    print(f"  (Python dict overwrite; last record iterated wins) is used, not a")
    print(f"  central-tendency aggregate. This means the 6,469 pair count and 2,578")
    print(f"  sign-flip count are NOT invariant to input record ORDER for these cells.")
...
print(f"C1 panels backed by >1 ChEMBL assay_id (i.e. the panel MERGES multiple")
print(f"  distinct ChEMBL assay records under one protocol signature -- this is")
print(f"  the entire point of GDR-011 Option D, but it also means a 'replicate'")
print(f"  within this panel may be two DIFFERENT physical assay instances, not")
...
print(f"  backed by a SINGLE ChEMBL assay_id (true repeat measurement):   {single_assay_replicates}")
print(f"  backed by MULTIPLE ChEMBL assay_ids (cross-assay agreement, NOT")
...
if sds_multi:
    print(f"  median sigma, MULTI-assay_id  replicate groups (n={len(sds_multi)}): "
...
for iso in sorted(by_iso_sds):
    sds = by_iso_sds[iso]
    print(f"  {iso:<12} n_groups={len(sds):4}  median={statistics.median(sds):.3f}  "
...
for r in acc:
    ...
    if fam: panel_cmpd_scaf[key][ik] = fam
...
if all_deltas:
    ...
    print(f"  median within-noise-floor (2x median C1 sigma = "
...
if all_deltas_noflip:
    print(f"Non-flip |Δselectivity| median: {statistics.median(all_deltas_noflip):.3f} (n={len(all_deltas_noflip)})")
