...
"""
import json, gzip, pathlib, re, statistics
...
with gzip.open("data/snapshots/activity_snapshot_A3/records.json.gz","rt") as f: recs=json.load(f)
...
for tdir in sorted(pathlib.Path("data/raw/chembl").iterdir()):
    ...
    for p in sorted((tdir/"IC50").glob("page_*.json")):
        for a in json.loads(p.read_text())["activities"]:
            meta[str(a.get("activity_id"))]=(a.get("bao_format"),a.get("assay_type"),a.get("assay_description") or "")
for r in acc:
    bf,at,d=meta.get(str(r.get("source_record_id")),(None,None,""))
    r["_bao"],r["_atype"]=bf,at
    m=ATP.search(d); r["_atp"]=float(m.group(1)) if m else None
...
KEYS={"A_document":lambda r:(r.get("study_id"),),
      "C1_protocol":lambda r:(r.get("study_id"),r["_bao"],r["_atype"]),
      "C2_protocol_ATP":lambda r:(r.get("study_id"),r["_bao"],r["_atype"],
...
def complete_set(keyfn):
    ...
    for r in acc:
        ik=r.get("inchikey")
        if ik: g[keyfn(r)][ik].add(r.get("isoform"))
    out=set()
    for _,cm in g.items():
        for ik,iso in cm.items():
            if iso>=T1: out.add(ik)
    return out

print("=== POTENCY / SELECTION BIAS ===")
allp=[pact(r) for r in acc]; allp=[x for x in allp if x is not None]
print(f"  FULL corpus      n={len(allp):6}  median pAct={statistics.median(allp):.2f}  "
...
for name,kf in KEYS.items():
    s=complete_set(kf); sets[name]=s
    sub=[pact(r) for r in acc if r.get("inchikey") in s]; sub=[x for x in sub if x is not None]
    if not sub: continue
    print(f"  {name:<16} n={len(sub):6}  median pAct={statistics.median(sub):.2f}  "
...
for name,kf in KEYS.items():
    s=sets[name]
    if not s: print(f"  {name:<16} EMPTY"); continue
    ...
    for r in acc:
        ...
        if ik in s and pact(r) is not None:
            ...
            if r.get("scaffold_family_id"): scaf[ik]=r["scaffold_family_id"]
    pairs=0; switches=0
    for g,cm in grp.items():
        comp=[ik for ik,d in cm.items() if set(d)>=T1]
        by_scaf=defaultdict(list)
        for ik in comp:
            if ik in scaf: by_scaf[scaf[ik]].append(ik)
...
for name,kf in KEYS.items():
    rg=defaultdict(list)
    for r in acc:
        v=pact(r); ik=r.get("inchikey")
        if v is not None and ik: rg[(kf(r),ik,r.get("isoform"))].append(v)
    multi=[v for v in rg.values() if len(v)>=2]
    if not multi: print(f"  {name:<16} no replicate groups"); continue
    sds=[statistics.stdev(v) for v in multi]
    print(f"  {name:<16} groups={len(multi):5}  obs={sum(len(v) for v in multi):6}  "
