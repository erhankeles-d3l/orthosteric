...
fields = ["assay_type","bao_format","bao_label","assay_description","assay_variant_mutation",
          "target_organism","standard_units","standard_type","data_validity_comment","activity_properties"]
present = Counter(); total = 0
    ...
    for p in sorted((tdir/"IC50").glob("page_*.json")):
        for a in json.loads(p.read_text())["activities"]:
            ...
            for f in fields:
                ...
                if v not in (None, "", [], {}):
                    present[f] += 1
                    if f in ("assay_type","bao_format","bao_label","standard_units","target_organism"):
                        vals[f][str(v)] += 1
...
for aid, m in assay_meta.items():
    if m["doc"]: doc_assays[m["doc"]].append(m)
...
for d in four_docs:
    ...
    same_type += st; same_bao += sb; same_both += (st and sb)
print(f"  all assays share assay_type : {same_type}/{len(four_docs)} ({100*same_type/len(four_docs):.1f}%)")
print(f"  all assays share bao_format : {same_bao}/{len(four_docs)} ({100*same_bao/len(four_docs):.1f}%)")
print(f"  share both                  : {same_both}/{len(four_docs)} ({100*same_both/len(four_docs):.1f}%)")
...
for d in four_docs:
    ...
    if not known: none_known += 1
    elif len(known) < len(atps): partial += 1
    elif len(set(known)) == 1: allknown += 1
    else: conflict += 1
print(f"\n=== ATP consistency within 4-isoform documents ===")
print(f"  all 4 assays state SAME ATP conc : {allknown}/{len(four_docs)} ({100*allknown/len(four_docs):.1f}%)")
print(f"  all 4 state ATP but CONFLICTING  : {conflict}/{len(four_docs)} ({100*conflict/len(four_docs):.1f}%)")
print(f"  only SOME assays state ATP       : {partial}/{len(four_docs)} ({100*partial/len(four_docs):.1f}%)")
print(f"  no assay states ATP              : {none_known}/{len(four_docs)} ({100*none_known/len(four_docs):.1f}%)")
