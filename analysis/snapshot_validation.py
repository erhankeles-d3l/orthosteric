...
print(
    f"{'snapshot':<10}{'id':<20}{'recs':>7}{'cnt=man':>9}{'sha repro':>11}{'4 iso':>7}{'parent':<16} verdict"
)
print("-" * 100)
for d in sorted(pathlib.Path("data/snapshots").iterdir()):
    ...
    b = SnapshotBuilder(
        software=SoftwareProvenance(
            python_version=sw["python_version"],
            rdkit_version=sw["rdkit_version"],
            orthosteric_version=sw["orthosteric_version"],
            git_sha=sw["git_sha"],
            git_dirty=sw["git_dirty"],
            os_platform=sw["os_platform"],
            os_version=sw["os_version"],
            lockfile_hash=sw.get("lockfile_hash", ""),
            key_package_versions=sw.get("key_package_versions", {}),
        ),
    ...
    if d.name.endswith("A0"):
        verdict += " (VOID per ADR-0013)"
    print(
        f"{d.name.replace('activity_snapshot_', ''):<10}{man['snapshot_id']:<20}{len(recs):>7}"
        f"{cnt_ok!s:>9}{repro!s:>11}{four!s:>7} {par:<15} {verdict}"
    )
