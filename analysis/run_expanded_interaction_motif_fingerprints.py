"""Real interaction-motif fingerprint pipeline for the EXPANDED 50-compound
production run. Identical pipeline to
analysis/run_interaction_motif_fingerprints.py (24-compound version) --
same receptors, same protocol (seed=42, exhaustiveness=8, 5 poses),
same modules -- only the compound selection file changes. Outputs go to
distinct files so the 24-compound run's provenance is preserved
alongside this expansion, not overwritten.
...
POSE_DIR = WORKDIR / "occupancy_poses_expanded"
...
print("=== Step 1: Real sequence-based residue correspondence table (reused, unchanged) ===")
...
for iso, recs in correspondence_table.by_target_isoform.items():
    ...
    print(f"  PI3Kalpha -> {iso}: {n_mapped}/{len(recs)} mapped ({100*n_mapped/len(recs):.1f}%)")
...
for i, c in enumerate(compounds):
    ...
    if not ligand_pdbqt.exists():
        n_failed += 4
        print(f"  {cid[:16]}: LIGAND MISSING, skipping all 4 isoforms")
        continue
...
fp_out = Path("/home/ubuntu/Documents/orthosteric/data/structural_evidence/expanded_interaction_motif_fingerprints.json")
fp_out.write_text(json.dumps(
    {cid: {iso: fp.to_dict() for iso, fp in by_iso.items()} for cid, by_iso in all_fingerprints.items()},
    indent=2,
))
comp_out = Path("/home/ubuntu/Documents/orthosteric/data/structural_evidence/expanded_comparative_interaction_fingerprints.json")
